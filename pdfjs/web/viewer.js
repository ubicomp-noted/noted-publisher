...
var PDFPageView = (function PDFPageViewClosure() {
  ...
  PDFPageView.prototype = {
    ...
    cssTransform: function PDFPageView_transform(canvas, redrawAnnotations) {
      ...
      var div = this.div;
      canvas.parentNode.style.width = Math.floor(width) + 'px';
      canvas.style.width = div.style.width =
        (Math.floor(width) - ANNOTATION_MARGIN) + 'px';
    ...
    draw: function PDFPageView_draw() {
      ...
      saveButton.id = 'saveButton' + this.id;
      saveButton.addEventListener("click", function () {
        BOUNDING_BOXES[ACTIVE_BB].data = document.getElementById("annotationText" + this.id.substring(10, this.id.length)).innerHTML;
        fs.writeFile(path.resolve(__dirname, 'examples/bb_example.json') , JSON.stringify(BOUNDING_BOXES));
      }, false);

      var annotation = document.createElement('p');
      annotation.style.outline = "0px solid transparent";
      annotation.contentEditable = true;
      annotation.id = 'annotationText' + this.id;
      //saveButton.style.top = (parseInt(div.style.height.slice(0, -2)) - 70) + 'px'
      ...
      for (var bbb = 0; bbb < BOUNDING_BOXES.length; bbb++) {
        ...
        if (BOUNDING_BOXES[bbb].page == this.id) {
          ...
          boundingBoxes[bbb].addEventListener("click", function () {
            var curPage = BOUNDING_BOXES[this.id.substring(3, this.id.length)].page;
            var annote = document.getElementById("annotationText" + curPage);
            annote.innerHTML = BOUNDING_BOXES[this.id.substring(3, this.id.length)].data;
            var saveButton = document.getElementById("saveButton" + curPage);
            saveButton.style.visibility = 'visible';
            ACTIVE_BB = parseInt(this.id.substring(3, this.id.length));
          });
